...
// Tampil Mata Pelajaran
  default:
    if ($_SESSION[leveluser]=='admin'){
      ...
      echo "<h2>Manajemen Mata Kuliah</h2><hr>
          ...
          <tr><th>No</th><th>Id Matkul</th><th>Nama</th><th>Kelas</th><th>Dosen</th><th>Deskripsi</th><th>Aksi</th></tr></thead>";
    $no=1;
  ...
  if (!empty($cek_mapel)){
    echo"<h2>Mata kuliah Yang Anda Ambil</h2><hr>
    ";
    echo "<br><br><table id='table1' class='gtable sortable'><thead>
          <tr><th>No</th><th>Nama</th><th>Kelas</th><th>Dosen</th><th>Deskripsi</th><th>Aksi</th></tr></thead>";
    $no=1;
    while ($r=mysql_fetch_array($tampil_pelajaran)){
       echo "<tr><td>$no</td>             
             <td>$r[nama]</td>";
    ...
    elseif ($_SESSION[leveluser]=='siswa'){
        ...
        echo "<table>
          <tr><th>No</th><th>Nama</th><th>Dosen</th><th>Deskripsi</th></tr>";
        $no=1;
        while ($r=mysql_fetch_array($tampil_pelajaran)){
        echo "<tr><td>$no</td>
             <td>$r[nama]</td>";             
...
case "tambahmatapelajaran":
    if ($_SESSION[leveluser]=='admin'){
        echo "<form method=POST action='$aksi?module=matapelajaran&act=input_matapelajaran'>
         ...
         <dt><label>Dosen</label></dt>              <dd>: <select name='id_pengajar'>
                                                  <option value=0 selected>--pilih--</option>";
    ...
    elseif ($_SESSION[leveluser]=='pengajar'){
        echo "<form method=POST name='form_materi' action='$aksi?module=matapelajaran&act=input_matapelajaran_pengajar'>          
          <fieldset>
          <legend>Mata kuliah yang di Ambil</legend>
          <dl class='inline'>
          <dt><label>Kelas </label></dt>             <dd><select name='id_kelas' onChange='showpel()'>
                                          ...
                                          echo"</select></dd>
          <dt><label>Mata kuliah </label></dt>          <dd><select name='id_matapelajaran' onChange='showpel()'>
                                          <option value=''>-pilih-</option>";
                                          $pilih="SELECT * FROM mata_pelajaran ORDER BY id_matapelajaran";
                                          $query=mysql_query($pilih);
                                          while($row=mysql_fetch_array($query)){
                                          echo"<option value='".$row[id_matapelajaran]."'>".$row[nama]."</option>";
                                          }
                                          echo"</select></dd>
    ...
    <br><input class='button blue' type=button value=Kembali onclick=self.history.back()>
      ...
      $detail =mysql_query("SELECT * FROM mata_pelajaran WHERE id_matapelajaran = '$_GET[id]'");
        echo "<span class='judulhead'><p class='garisbawah'>Detail Mata Pelajaran</p></span>
          <table>
          <tr><th>no</th><th>nama</th><th>kelas</th><th>dosen</th><th>deskripsi</th></tr>";
                    $no=1;
    while ($r=mysql_fetch_array($detail)){
       echo "<tr><td>$no</td>             
             <td>$r[nama]</td>";
