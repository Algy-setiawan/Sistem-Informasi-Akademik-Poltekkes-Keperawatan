...
switch($_GET[act]){
  ...
  default:
    if ($_SESSION[leveluser]=='admin'){
        ...
        echo "<h2>Manajemen Materi</h2><hr>
          <input class='button blue' type=button value='Tambah Materi Dan Tugas On Line' onclick=\"window.location.href='?module=materi&act=tambahmateri';\">";

        echo "<br><br><table id='table1' class='gtable sortable'><thead>
          <tr><th>No</th><th>Judul</th><th>Kelas</th><th>Mata Kuliah</th><th>Nama File</th><th>Tgl Posting</th><th>Pembuat</th><th>Hits</th><th>Aksi</th></tr></thead>";
       $no=1;
 ...
 elseif ($_SESSION[leveluser]=='pengajar'){        
    ...
    $cek_materi = mysql_query("SELECT * FROM file_materi WHERE pembuat = '$_SESSION[idpengajar]'");
     ...
     echo "<h2>Daftar Materi Dan Tugas Yang Anda Upload</h2><hr>
          <input class='button blue' type=button value='Tambah Materi Dan Tugas Offline' onclick=\"window.location.href='?module=materi&act=tambahmateri';\">";
     echo "<br><br><table id='table1' class='gtable sortable'><thead>
          <tr><th>No</th><th>Judul</th><th>Kelas</th><th>Matkul</th><th>Nama File</th><th>Tgl Upload</th><th>Hits</th><th>Aksi</th></tr></thead>";
    ...
    echo"</table>";   
        ...
        echo"<br><b class='judul'>Materi Dan Tugas Offline</b><br><p class='garisbawah'></p>";
       ...
       echo "<table>
          <tr><th>No</th><th>Mata Kuliah</th><th>Materi/Tugas Offline</th></tr>";
        $no=1;
        while ($r=mysql_fetch_array($mapel)){
        echo "<tr><td>$no</td>
             <td>$r[nama]</td>";
             echo "<td><input type=button class='tombol' value='Lihat File Materi/Tugas Offline'
...
case "tambahmateri":
    ...
    echo "<form name='form_materi' method=POST action='$aksi?module=materi&act=input_materi' enctype='multipart/form-data'>
     <fieldset>
     <legend>Tambah Materi Dan Tugas Offline</legend>
     <dl class='inline'>
    ...
    <dt><label>Kelas</label></dt>              <dd> <select name='id_kelas' onChange='showpel_pengajar()'>
                                          ...
                                          echo"</select></dd>
    <dt><label>Mata Kuliah</label></dt>          <dd> <div id='pelajaran_pengajar'></div></dd>
...
case "editmateri":
    ...
    <dt><label>Kelas</label></dt>              <dd>: <select name='id_kelas' onChange='showpel_pengajar()'>
                                          ...
                                          echo"</select></dd>
    <dt><label>Mata Kuliah</label></dt>          <dd>: <select id='pelajaran_pengajar' name='id_matapelajaran'>
                                          <option value='".$p[id_matapelajaran]."' selected>".$p[nama]."</option>
                                          </select></dd>
    <dt><label>File</label></dt>              <dd>: $m[nama_file]</dd>
    <dt><label>Ganti File</label></dt>        <dd>: <input type=file name='fupload' size=40>
